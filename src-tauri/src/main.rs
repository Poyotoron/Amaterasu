...
fn controller_loop(app_handle: AppHandle) {
    ...
    // 初期カウントの送信
    app_handle
        .emit("buttonCounter", controller.get_button_count())
        .unwrap();
    app_handle
        .emit("scratchCount", controller.get_scratch_count())
        .unwrap();
    ...
    loop {
        ...
        if controller.get_button_diff() {
            app_handle
                .emit("buttonState", controller.get_button_state())
                .expect("failed to send controller button state");
        }
        if scratch_send_counter >= scratch_send_interval {
            app_handle
                .emit("scratchState", controller.get_scratch_state())
        ...
        if controller.button_pressed_all(vec![
            ...
            if !is_pause_toggled {
                ...
                app_handle.emit("togglePause", is_pause).unwrap();
            }
        } else {
        ...
        if controller.button_pressed_all(vec![
            ...
            if !is_2p_toggled {
                is_2p = !is_2p;
                is_2p_toggled = true;
                app_handle.emit("toggle2P", is_2p).unwrap();
            }
        } else {
            is_2p_toggled = false;
        }
        ...
        if controller.button_pressed_all(vec![
            ...
            if !is_save_pressed {
                ...
                app_handle.emit("savedCount", true).unwrap();
            }
        } else {
